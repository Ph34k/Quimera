...
# ⚡ Bolt Optimization: Use a persistent httpx.Client to enable connection pooling.
# This avoids the overhead of establishing a new TCP/TLS connection on every request,
# significantly improving performance for repeated HTTP calls.
...
class IScoutAgent(BaseAgent):
    ...
    def execute(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            response = _http_client.get(target_url, timeout=5.0)
            return {
                "status": "success",
...
class IExecutionAgent(BaseAgent):
    ...
    def execute(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            resp = _http_client.get(target_url, headers=headers, timeout=5.0, follow_redirects=True)
            return {
