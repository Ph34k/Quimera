...
# SECURITY: Validate target URL to prevent SSRF (Server-Side Request Forgery)
def is_safe_url(url: str) -> bool:
    try:
        ...
        for info in addrinfo:
            ...
            # Ensure the IP is public/global, blocking local and cloud metadata IPs
            if not ip.is_global:
                return False
        return True
    except Exception:
...
class IScoutAgent(BaseAgent):
    ...
    def execute(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            # SECURITY: follow_redirects=False prevents SSRF via redirect to local IPs
