...
def is_safe_url(url: str) -> bool:
    """Validates if a URL points to a safe public IP address."""
    try:
        parsed = urlparse(url)
        ...
        # Resolve hostname to IP using getaddrinfo to handle IPv4/IPv6 and avoid some DNS rebinding
...
def verify_request(request: httpx.Request):
    """httpx event hook to verify URL before request."""
    if not is_safe_url(str(request.url)):
        raise httpx.RequestError(f"Unsafe target URL detected: {request.url}", request=request)
...
class IScoutAgent(BaseAgent):
    ...
    def execute(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            # Real HTTP request with SSRF protection
