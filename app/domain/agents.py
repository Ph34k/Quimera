...
def is_safe_url(url_str: str) -> bool:
    try:
        ...
        for _, _, _, _, sockaddr in addr_info:
            ...
            if parsed_ip.is_private or parsed_ip.is_loopback or parsed_ip.is_link_local or parsed_ip.is_unspecified:
                return False
        return True
    except Exception:
        return False
...
def verify_request(request: 'httpx.Request'):
    if not is_safe_url(request.url):
        raise httpx.RequestError(f"URL is not safe: {request.url}", request=request)
...
class IScoutAgent(BaseAgent):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.client = httpx.Client(event_hooks={'request': [verify_request]}, follow_redirects=True, timeout=5.0)
