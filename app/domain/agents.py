...
def _is_safe_url(url: str) -> bool:
    """Security Helper: Validates if a URL is safe to prevent SSRF."""
    try:
    ...
    def execute(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not _is_safe_url(target_url):
            return {
                ...
                "target": target_url,
                "error": "Security Error: Target URL resolves to a disallowed private or invalid IP."
            }

        try:
            # MVP: Real HTTP request instead of mock, disabled redirects for SSRF
...
class IExecutionAgent(BaseAgent):
    ...
    def execute(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not _is_safe_url(target_url):
            return {"status": "execution_failed", "error": "Security Error: Target URL resolves to a disallowed private or invalid IP."}
