...
# ⚡ Bolt: Global httpx Client for connection pooling.
# Using a shared client avoids TCP handshake overhead on every request,
# reducing latency for external calls.
_http_client = httpx.Client(timeout=5.0, follow_redirects=True)
...
class IScoutAgent(BaseAgent):
    ...
    def execute(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            # ⚡ Bolt: Use shared client for connection pooling
...
class IExecutionAgent(BaseAgent):
    ...
    def execute(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            # ⚡ Bolt: Use shared client for connection pooling
            resp = _http_client.get(target_url, headers=headers)
            return {
