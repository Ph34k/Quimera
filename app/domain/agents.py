...
class IScoutAgent(BaseAgent):
    ...
    def execute(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not is_safe_url(target_url):
            return {"status": "failed", "mission_id": str(uuid.uuid4()), "target": target_url, "error": "SSRF Blocked: Invalid or internal URL"}

        try:
            # MVP: Real HTTP request instead of mock
            # SSRF Protection: Do not follow redirects
...
class IExecutionAgent(BaseAgent):
    ...
    def execute(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not is_safe_url(target_url):
            return {"status": "execution_failed", "error": "SSRF Blocked: Invalid or internal URL"}

        headers = {"User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36"}
        try:
            # SSRF Protection: follow_redirects=False prevents redirecting to internal IP
